...
# Author:           Travis Bubb
# Description:      The player class to play Dots and Boxes
...
class Player:
    ...
    def get_move(self, state):
        ...
        while True:
            ...
            if len(move) != 2:
                print('Incorrect number of inputs. Please enter 2 integer values separated by a space.')
                valid = False
                continue
            
            # check that the input is only integer values
            for s in move:
                if not(s.isdigit() and int(s) >= 0 and int(s) <= (state['board_size'][0] * 2) + 1):
                    print('Please only enter integer values in the proper range of the board.')
                    valid = False
                    are_ints = False
                    break
            ...
            if are_ints:
                ...
                if tuple(move) not in moves:
                    print(f'({move[0]}, {move[1]}) is not a valid move. Please enter a new move.')
                    valid = False
                    continue
